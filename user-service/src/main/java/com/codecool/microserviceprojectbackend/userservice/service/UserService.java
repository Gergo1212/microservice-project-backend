...
public class UserService {
    ...
    public boolean registerUser(UserModel userCredentials) {
        ...
        if(!existsByUsername) {
            userRepository.save(UserEntity.builder().username(userCredentials.getUsername()).password(passwordEncoder.encode(userCredentials.getPassword())).roles(Collections.singletonList("ROLE_USER")).build());
            return true;
        }
        return false;
    }
